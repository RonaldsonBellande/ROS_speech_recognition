...
public:
  RosGstPlay() {
    ...
    g_object_set(G_OBJECT(_source), "do-timestamp",
                 (do_timestamp) ? TRUE : FALSE, NULL);

    //_playbin = gst_element_factory_make("playbin2", "uri_play");
    // g_object_set( G_OBJECT(_playbin), "uri", "file:///home/test/test.mp3",
    // NULL);
    caps = gst_caps_new_simple(
        "audio/x-raw", "format", G_TYPE_STRING, sample_format.c_str(), "rate",
        G_TYPE_INT, sample_rate, "channels", G_TYPE_INT, channels, "width",
        G_TYPE_INT, depth, "depth", G_TYPE_INT, depth, "signed", G_TYPE_BOOLEAN,
    ...
    if (format == "mp3") {
      if (dst_type == "alsasink") {
        ...
        gst_bin_add_many(GST_BIN(_pipeline), _source, _decoder, _filter, _audio,
                         NULL);

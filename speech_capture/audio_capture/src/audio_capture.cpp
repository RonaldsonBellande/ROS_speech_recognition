...
public:
  RosGstCapture() {
    ...
    _pub_info =
        _nh.advertise<audio_common_msgs::AudioInfo>("audio_info", 1, true);
    ...
    if (dst_type == "appsink") {
      ...
      g_signal_connect(G_OBJECT(_sink), "new-sample", G_CALLBACK(onNewBuffer),
                       this);
    } else {
    ...
    caps = gst_caps_new_simple(
        "audio/x-raw", "format", G_TYPE_STRING, _sample_format.c_str(),
        "channels", G_TYPE_INT, _channels, "width", G_TYPE_INT, _depth, "depth",
        G_TYPE_INT, _depth, "rate", G_TYPE_INT, _sample_rate, "signed",
    ...
    if (_format == "mp3") {
      ...
      gst_bin_add_many(GST_BIN(_pipeline), _source, _filter, _convert, _encode,
                       _sink, NULL);
      link_ok = gst_element_link_many(_source, _filter, _convert, _encode,
                                      _sink, NULL);
  ...
  void publish(const audio_common_msgs::AudioData &msg) { _pub.publish(msg); }
  ...
  static gboolean onMessage(GstBus *bus, GstMessage *message,
                            gpointer userData) {
    RosGstCapture *server = reinterpret_cast<RosGstCapture *>(userData);
    GError *err;
    gchar *debug;
